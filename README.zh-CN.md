...
1. **调整配置** 在 `docker-compose.yml` 文件中：
   ...
   - 根据需要更新子网和网关
   - （可选）为生产环境设置 JWT 密钥
   - **容器部署特别说明**：服务启动后，请通过 Web 界面进入 **设置 → WebRTC 设置**，将 **公网 IP / Public IP** 配置为**浏览器可以访问到的 IP**，通常为宿主机的局域网 IP（家庭/局域网环境）或服务器的公网 IP（对外提供服务时）。
   - **注意**：WebRTC 和 TURN 服务器的其他参数也可以在部署后通过 Web 界面配置（设置 → WebRTC 设置 / TURN 服务器设置）
...
- 容器环境下部署时，**必须正确设置 `PublicIp` 为浏览器可访问的地址**（宿主机局域网 IP 或服务器公网 IP），否则后端生成的 WebRTC 候选地址可能不可用
