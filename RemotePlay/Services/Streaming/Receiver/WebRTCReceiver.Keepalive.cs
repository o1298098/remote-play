...
namespace RemotePlay.Services.Streaming.Receiver
{
    ...
    public sealed partial class WebRTCReceiver
    {
        ...
        private UdpClient? _turnKeepaliveUdpSocket; // 用于发送 UDP TURN keepalive 的独立 UDP socket
        private TcpClient? _turnKeepaliveTcpSocket; // 用于发送 TCP TURN keepalive 的独立 TCP socket
        ...
        private void StartTurnKeepalive()
        {
            ...
            try
            {
                ...
                {
                    // ✅ UDP TURN: 使用 UDP socket 向 relay 地址发送
                    if (_turnKeepaliveUdpSocket == null)
                    {
                        _turnKeepaliveUdpSocket = new UdpClient();
                    }
                    ...
                    _turnKeepaliveTask = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            {
                                if (_turnRelay != null && _turnKeepaliveUdpSocket != null && !_disposed)
                                {
                                    try
                                    {
                                        var stunPacket = BuildStunBindingRequest();
                                        await _turnKeepaliveUdpSocket.SendAsync(stunPacket, stunPacket.Length, _turnRelay);
                                        
                                        // 每 20 次记录一次日志（避免日志过多）
                                        if (_videoPacketCount % 20 == 0)
                                        {
                                            _logger.LogDebug("✅ UDP TURN keepalive 已发送到 {Relay}", _turnRelay);
                                        }
                                    }
                                    catch (Exception ex)
                                    {
                                        _logger.LogWarning(ex, "⚠️ UDP TURN keepalive 发送失败: {Relay}", _turnRelay);
                                    }
                                }
                        ...
                        {
                            _logger.LogError(ex, "❌ UDP TURN keepalive 循环异常");
                        }
                    }, _turnKeepaliveCts.Token);
                    
                    _logger.LogInformation("✅ UDP TURN keepalive 已启动 (间隔: {Interval}ms, Relay: {Relay})", 
