...
namespace RemotePlay.Services.Streaming
{
    ...
    public class FeedbackSenderService : IDisposable
    {
            ...
            Func<int, ushort, byte[], Task> sendFeedbackFunc)
        {
            ...
            // 初始化为 idle 状态
            _currentState = ControllerState.CreateIdle();
            _previousState = ControllerState.CreateIdle();
        }
        ...
        private async Task FeedbackLoopAsync(CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                try
                {
                    ...
                    ControllerState currentStateCopy;
                    ControllerState previousStateCopy;
        ...
        /// </summary>
        private async Task SendFeedbackStateAsync(ControllerState state)
        {
            // ✅ PS5 格式：28 字节 (0x1c)
            // ✅ PS4 格式：25 字节 (0x19)
            // 这里使用 PS5 格式
        ...
        /// </summary>
        private async Task SendFeedbackHistoryAsync(ControllerState prev, ControllerState current)
        {
        ...
        /// （只比较摇杆和陀螺仪，不比较按键）
        /// </summary>
        private bool StatesEqualForFeedbackState(ControllerState a, ControllerState b)
        {
            // 摇杆
        ...
        /// （只比较按键和扳机）
        /// </summary>
        private bool StatesEqualForFeedbackHistory(ControllerState a, ControllerState b)
        {
            // 按键
