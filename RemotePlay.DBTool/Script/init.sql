...
CREATE TABLE t_base_enum (
    ...
    CONSTRAINT "PK_t_base_enum" PRIMARY KEY (id)
);
...
CREATE TABLE t_base_log (
    ...
    properties jsonb,
    created_at timestamp with time zone NOT NULL,
    CONSTRAINT "PK_t_base_log" PRIMARY KEY (id)
);
...
CREATE TABLE t_user_device (
    ...
    CONSTRAINT foreign_k_userdevice_k FOREIGN KEY (device_id) REFERENCES t_playstation_device (id) ON DELETE CASCADE
);

INSERT INTO t_base_enum (id, created_at, description, enum_code, enum_key, enum_type, enum_value, is_active, sort_order, updated_at)
...
CREATE INDEX "IX_t_base_enum_enum_type" ON t_base_enum (enum_type);

CREATE UNIQUE INDEX "IX_t_base_enum_enum_type_enum_key" ON t_base_enum (enum_type, enum_key);

CREATE INDEX "IX_t_base_enum_sort_order" ON t_base_enum (sort_order);

CREATE INDEX "IX_t_base_log_created_at" ON t_base_log (created_at);

CREATE INDEX "IX_t_base_log_device_id" ON t_base_log (device_id);

CREATE INDEX "IX_t_base_log_level_created_at" ON t_base_log (level, created_at);

CREATE INDEX "IX_t_base_log_user_id" ON t_base_log (user_id);

CREATE INDEX "IX_t_device_config_device_id" ON t_device_config (device_id);

CREATE UNIQUE INDEX "IX_t_device_config_user_id_device_id_config_key" ON t_device_config (user_id, device_id, config_key);
...
VALUES ('20251110154442_init', '9.0.10');
